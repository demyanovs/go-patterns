...
Например, если будут куплены 7 товаров одного вида по цене 2 за штуку и 5
товаров другого вида по цене 3 за штуку, то вместо 7*2+5*3 надо будет оплатить 5*2+4*3=22.
...
func main() {
	...
	//fmt.Fprintln(out, "total sets: ", testCount)

	for i := 0; i < testCount; i++ {
		var c int
		fmt.Fscan(in, &c)
		//fmt.Fprintln(out, "products count: ", c)

		var products = make([]int, c)
		fmt.Fscan(in, packAddrs(products)...)
		//fmt.Fprintln(out, "products: ", products)
		var productsByPrice = make(map[int]int)
		var sum int
		for _, p := range products {
			productsByPrice[p]++
			if productsByPrice[p]%3 == 0 {
				productsByPrice[p] = 0
				continue
			}
			sum += p
		...
		//fmt.Fprintln(out, "sum of products: ", sum)
